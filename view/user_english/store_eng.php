...
try {
    ...
    if ($fecha_nacimiento instanceof DateTime) {
        ...
        $error = "Error creating DateTime object from date of birth.";
    }
} catch (Exception $e) {
    // Captura cualquier excepción lanzada al procesar la fecha
    $error = "Exception when processing date of birth: " . $e->getMessage();
}
$edad = $fecha_nacimiento->diff(new DateTime())->y;
...
if ($contraseña == $confirmarContraseña) {
    ...
    if ($registradoEmail) {
        ...
        $verificationLink = "http://localhost/move-it/view/user_english/verificar_eng.php?token=" . $token;
