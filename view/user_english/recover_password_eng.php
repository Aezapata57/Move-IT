...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($emailExists) {
        ...
        $recoveryLink = "http://localhost/MOVE-IT/view/user_english/reset_password_eng.php?token=" . $token;
        ...
        if ($mail->send()) {
            ...
            header("Location: recover_eng.php?message=" . $message);
            exit();
        } else {
            $error = "<li>Error sending email: " . $mail->ErrorInfo . " .Try again</li>";
            header("Location: recover_eng.php?error=". $error);
        }
    } else {
